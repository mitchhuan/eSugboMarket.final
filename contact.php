...
$selectedOrderId = isset($_GET['order_id']) ? $_GET['order_id'] : null; // Added this line
$messages = array();
...
if ($selectedUserId && $selectedOrderId) {
    ...
    $query = $conn->prepare("SELECT sender_id, receiver_id, order_id, message_content, timestamp FROM messages WHERE
        ...
        (sender_id = ? AND receiver_id = ?)) AND order_id = ?  -- Added this line
        ORDER BY timestamp ASC");
    $query->execute([$selectedUserId, $user_id, $user_id, $selectedUserId, $selectedOrderId]);
...
$users = array();
$queryUsers = $conn->prepare("SELECT DISTINCT m.sender_id AS cour_id, u.name, u.image
                             FROM messages m
                             ...
                             UNION
                             SELECT DISTINCT m.receiver_id AS cour_id, u.name, u.image
                             FROM messages m
...
$queryUsers->execute([$_SESSION['user_id'], $_SESSION['user_id'], $_SESSION['user_id'], $_SESSION['user_id'], $_SESSION['user_id'], $_SESSION['user_id']]);
...
$queryRecentUsers = $conn->prepare("SELECT DISTINCT m.sender_id AS cour_id, u.name, u.image, MAX(m.timestamp) as latest_message_time
FROM users u
INNER JOIN messages m ON (u.id = m.sender_id AND m.receiver_id = :currentUserId)
WHERE u.id != :currentUserId
GROUP BY m.sender_id, u.name, u.image
...
if (!$selectedUserId && !empty($users)) {
    $selectedUserId = $users[0]['cour_id'];
        ...
        <div class="message-container">
            <div class="users">
                ...
                <div class="search-bar">
                    <input type="text" id="user-search" placeholder="Search couriers">
                </div>
                <ul class="user-list">
                    <?php foreach ($recentUsers as $user) : ?>
                        <li <?php if (getUnreadMessageCount($conn, $currentUserId, $user['cour_id']) > 0) : ?> class="unread" <?php endif; ?>>
                            <a href="contact.php?user_id=<?= $user['cour_id'] ?>">
                                <div class="user-box">
            ...
            <div class="messages">
                ...
                <div class="message-display">
                    ...
                    ?>
                        <p>Select a courier to start a conversation.</p>
                    <?php
                    }
                    ?>
                </div>
